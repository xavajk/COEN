...
namespace coen79_lab8
{
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index) 
        {
            ...
            else
            {
                ...
                for (size_type block_item_index = 0; block_item_index < block_size; ++block_item_index)
                {
                    ...
                    // copy element from source to block
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            // allocate new block and insert entry into block
            *first_bp = new value_type [block_size];
            **first_bp = entry;

            // set front_ptr and back_ptr to same location
        ...
        {
            // allocate block above current first block
            --first_bp;
            *first_bp = new value_type [block_size];

            // set front pointer to last element in new block
        ...
        {
            // reserve more space for deque and call push_front
            reserve();
            push_front(entry);
        }
    }
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        {
            // insert entry into back pointer + 1
            ++back_ptr;
            *back_ptr = entry;
        }
        ...
        {
            // allocate block below current last block
            ++last_bp;
            *last_bp = new value_type [block_size];
            
            // set back pointer to first element of new block
            back_ptr = *last_bp;
    ...
    void deque<Item>::pop_front()
    {
        ...
        if (back_ptr == front_ptr) clear();

        // This is the last entry of the data block; move to the next block
        else if (front_ptr == ((*first_bp) + block_size - 1))
        {
            // delete block containing only first item in deque
            delete[] *first_bp;

            // set block to NULL and move first_bp to block pointer below
            *first_bp = NULL;
            ++first_bp;

            // set front pointer to beginning of new first_bp
            front_ptr = *first_bp;
        }

        // simply move front_ptr
        else ++front_ptr;
    }
    ...
    void deque<Item>::pop_back()
    {
        ...
        {
            // delete block containing only last item in deque
            delete[] *last_bp;

            // set block to NULL and move last_bp to block pointer above
            *last_bp = NULL;
            --last_bp;

            // set back_ptr to end of new last_ptr
            back_ptr = *last_bp + block_size - 1;
        }

        // simply move back_ptr
        else --back_ptr;
    }
